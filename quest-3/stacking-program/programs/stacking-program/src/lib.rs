...
declare_id!("BMZFogFhE3khNc9EtRmKXvKLvsdSmAY7Gcjs2SGFtsnU");
...
pub mod stacking_program {
    ...
    pub fn destake(ctx: Context<DeStake>) -> Result<()> {
        ...
        // Get the authority here? (signer)
        let signer: &[&[&[u8]]] = &[&[constants::VAULT_SEED, &[ctx.bumps.token_vault_account]]];
...
pub struct DeStake<'info> {
    ...
    // Those three accounts always needed in a token account
    pub mint: Account<'info, Mint>,
    pub token_program: Program<'info, Token>,
    pub system_program: Program<'info, System>,
